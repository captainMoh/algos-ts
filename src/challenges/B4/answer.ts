...
export default function ({ submissions }: { submissions: Submission[] }): MonthSubmission[] {
    ...
    const months: { [key: string]: MonthSubmission } = {}
    const monthSubmission: MonthSubmission[] = []

    submissions.forEach(submission => {
        const date = new Date(submission.submittedAt.split('T')[0])
        const month = date.getUTCMonth() + 1;


        if (!months[month.toString()]) {
            months[month.toString()] = { month: date.toISOString(), submissions: [] }
        }
    })
